...
export default {
  ...
  methods: {
    aggregateDimensions(verified_smart_contracts) {
      verified_smart_contracts.forEach((element) => {
        ...
        this.series[0].data = [
          ...this.series[0].data,
          element["Verified Smart Contracts"],
        ];
      });
      return [];
    },
  ...
  async mounted() {
    await this.aggregateDimensions(verified_smart_contracts);
